...
void MonHocManager::hienThiDanhSach() const
{
    ...
    {
        std::cout << std::left
                  << std::setw(10) << mh.getMaMonHoc()
                  << std::setw(34) << mh.getTenMonHoc()
                  << std::setw(12) << mh.getSoTinChi()
                  << mh.getSoTiet() << std::endl;
    }
}
...
bool MonHocManager::suaMonHoc(const std::string &maMon)
{
    ...
    std::string tenMoi = InputHelper::inputLine("Nhap ten mon moi: ");
    int soTinChiMoi = InputHelper::inputInt("Nhap so tin chi moi: ", 1, 10);
    int soTietMoi = InputHelper::inputInt("Nhap so tiet moi: ", 1, 50);
...
{
    std::sort(dsMonHoc.begin(), dsMonHoc.end(),
